...
def simple_relocation(choosers, relocation_rate, location_fname):
    ...
    record_change_sets('updated', {'table':choosers.name, 'column':location_fname,
                                           'data':unplaced_values})


def record_change_sets(change_type, change):
    """
    ...
    if orca.get_injectable('track_changes'):
        ...
        column = change['column'] if 'column' in change.keys() else None
        cs = ChangeSet(change['data'],
                       change_type,
                       change['table'],
                       column=column)
...
class ChangeSet(object):
    def __init__(self, data, operation, table, column=None):
        ...
        self.year = orca.get_injectable('year')
...
def register_choice_model_step(model_name, agents_name, choice_function):
    ...
    def choice_model_simulate(location_choice_models):
        ...
        record_change_sets('updated', {'table':model.choosers, 'column':choice_column,
                                       'data':choices})
