...
def simple_transition(tbl, rate, location_fname, set_year_built=False):
    ...
    # Change tracking
    record_change_sets('added', (tbl.name, copied))
...
def simple_relocation(choosers, relocation_rate, location_fname):
    """
    ...
    previous_location = choosers[location_fname].loc[chooser_ids]
    unplaced_values = pd.Series(-1, index=chooser_ids)
    choosers.update_col_from_series(location_fname, unplaced_values)

    record_change_sets('updated', (choosers.name, location_fname,
                                           unplaced_values))


## todo:  timestamp with each change-set
...
def record_add(year, model_step, change_data):
    ...
    record_change(year, 'added', (table_name, model_step), added_records)


def record_removal(year, model_step, change_data):
    table_name = change_data[0]
    removed_records = change_data[1]
    record_change(year, 'removed', (table_name, model_step), removed_records)
...
def record_update(year, model_step, change_data):
    ...
    record_change(year, 'updated', (table_name, column_name, model_step), updated_data)


def record_change(year, change_type, changer, changes):
    change_sets = orca.get_injectable('change_sets')
    change_sets[year][change_type][changer] = changes
    orca.add_injectable('change_sets', change_sets)
...
def register_simple_relocation_model(agents_name, relocation_rate):

    @orca.step('simple_%s_relocation' % agents_name)
    def simple_relocation_model():
        agents_table = orca.get_table(agents_name)
        print('Running {} relocation with {:.2f} relocation rate'
              .format(agents_name, relocation_rate * 100.0))
        return simple_relocation(agents_table, relocation_rate,
                                 orca.get_injectable('geography_id'))

    return simple_relocation_model
...
def register_choice_model_step(model_name, agents_name, choice_function):
    ...
    def choice_model_simulate(location_choice_models):
        ...
        agents_table.update_col_from_series(
            choice_column, choices, cast=True)

        record_change_sets('updated', (model.choosers, choice_column,
                                       choices))
