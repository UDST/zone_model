...
def to_frame(tbl, join_tbls, cfg, additional_columns=[]):
    ...
    cfg : str or dict
        The filename of a yaml configuration file or dict of the configuration from 
        which to parse the strings which are actually used by the model
        ...
        try:
            ...
            try:
                ...
                if orca.get_injectable('multiprocess'):
                    ...
                    r.set('failed_for', cfg)
                cfg = yaml_to_class(cfg).from_yaml(str_or_buffer=cfg)
    else:
        cfg = yaml_to_class(cfg).from_yaml(yaml_str=yamlio.ordered_yaml(cfg))
...
def yaml_to_class(cfg):
    ...
    ----------
    cfg : str or dict
        The name of the yaml configuration file or the model configuration as a dict
...
def hedonic_simulate(cfg, tbl, join_tbls, out_fname):
    ...
    # Change tracking
    record_change_sets('updated', (tbl.name, out_fname, price_or_rent))
...
def lcm_simulate(cfg, choosers, buildings, join_tbls, out_fname,
    ...
    ----------
    cfg : string or dict
        The name of the yaml config file or dict from which to read the location
        choice model
    choosers : DataFrameWrapper
        A dataframe of agents doing the choosing
    buildings : DataFrameWrapper
    ...
    alternative_ratio : float, optional
        ...
        the alternatives will be sampled to meet this ratio (for performance reasons).
    """

    cfg = misc.config(cfg) if type(cfg) is str else cfg
    ...
    vacant_units = vacant_units[vacant_units.index.values >= 0]  ## must have positive index (otherwise problem w/ vacant_job_spaces)
    ...
    if enable_supply_correction is not None:
        ...
        if type(cfg) is str:
            ...
            lcm = yaml_to_class(cfg).from_yaml(yaml_str=yamlio.ordered_yaml(cfg))
    ...
    if type(cfg) is str:
        ...
        new_units, _ = yaml_to_class(cfg).predict_from_cfg(movers, units, cfg=yamlio.ordered_yaml(cfg), alternative_ratio=alternative_ratio)
    ...
    # Change tracking
    record_change_sets('updated', (choosers.name, out_fname, new_units))
...
def unplace(agents, num_agents, geogs, filters):
    """
    Randomly unplace the specified number of agents from the specified geographies

    Parameters
    ----------
    agents : string
        The name of the type of agents to unplace (e.g. residential_units, households,
        jobs)
    num_agents : integer
        The number of agents to unplace
    geogs : list or array of strings
        A list or array of zone_ids from which to unplace agents
    filters : string
        A string representing a dataframe.query (e.g. 'sector_id==21')
    """

    import random
    df = orca.get_table(agents).local
    print "unplacing %d %s from %s" %(num_agents, agents, geogs[:5])
    sub_df = df[df[geography_base_id].isin(geogs)]
    if len(filters) > 0:
        sub_df = sub_df.query(filters)
    unplace_ids = random.sample(sub_df.index, int(num_agents))
    df[geography_base_id][df.index.isin(unplace_ids)] = -1
    orca.add_table(agents, df)
        
    #randomly unplace households if not enough capacity
    if agents == 'residential_units':
        hh = orca.get_table('households').local
        b = orca.get_table('zones').to_frame('vacant_residential_units')
        b = b[b.vacant_residential_units<0]
        hh_unplace = []
        for i, row in b.iterrows():
            num_hh = np.abs(row.vacant_residential_units)
            sub_hh = hh[hh[geography_base_id]==i]
            hh_unplace.extend(random.sample(sub_hh.index, int(num_hh)))
        hh[geography_base_id][hh.index.isin(hh_unplace)] = -1
        orca.add_table('households', hh)


def place_res_units(zone_ids):
    """
    Use the zone_model rdplcm specs to place residential units 
    into a specified set of zones

    Parameters
    ----------
    zone_ids : list of strings
        A list of the zone ids into which to place unplaced residential units
    
    """
    
    ru = orca.get_table('residential_units').to_frame([geography_base_id,'building_type_id'])
    for b_type in ru.building_type_id[ru[geography_base_id]==-1].unique():
        cfg = './configs/rdplcm' + str(int(b_type)) + '.yaml'
        cfg = yamlio.yaml_to_dict(str_or_buffer=cfg)
        cfg['alts_predict_filters'] = 'b_id in %s' % (zone_ids)
        utils.lcm_simulate(cfg, orca.get_table('residential_units'), orca.get_table('zones'),
                           [], geography_base_id, 'du_spaces', 'vacant_du_spaces', alternative_ratio=np.inf)


def place_households(zone_ids):
    """
    Use the zone_model hlcm specs to place households 
    into a specified set of zones

    Parameters
    ----------
    zone_ids : list of strings
        A list of the zone ids into which to place unplaced households
    
    """
    
    hh = orca.get_table('households').to_frame([geography_base_id,'income_quartile'])
    for hh_type in hh.income_quartile[hh[geography_base_id]==-1].unique():
        cfg = './configs/hlcm' + str(int(hh_type)) + '.yaml'
        cfg = yamlio.yaml_to_dict(str_or_buffer=cfg)
        cfg['alts_predict_filters'] = 'zone_id in %s' % (zone_ids)
        utils.lcm_simulate(cfg, orca.get_table('households'), orca.get_table('zones'), [], geography_base_id, 
                           'residential_units', 'vacant_residential_units', alternative_ratio=np.inf)


def place_jobs(zone_ids):
    """
    Use the zone_model elcm specs to place jobs 
    into a specified set of zones

    Parameters
    ----------
    zone_ids : list of strings
        A list of the zone ids into which to place unplaced jobs
    
    """
    jobs = orca.get_table('jobs').to_frame([geography_base_id,'sector_id'])
    for job_type in jobs.sector_id[jobs[geography_base_id]==-1].unique():
        cfg = './configs/elcm' + str(int(job_type)) + '.yaml'
        cfg = yamlio.yaml_to_dict(str_or_buffer=cfg)
        cfg['alts_predict_filters'] = 'zone_id in %s' % (zone_ids)
        utils.lcm_simulate(cfg, orca.get_table('jobs'), orca.get_table('zones'),
                           [], geography_base_id, 'job_spaces', 'vacant_job_spaces', alternative_ratio=np.inf)
...
def full_transition(agents, agent_controls, year, location_fname,
    ...
    updated, added, copied, removed = tran.transition(hh, year)
