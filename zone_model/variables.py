...
def make_agg_var(agent, geog, geog_id, var_to_aggregate, agg_function):
    ...
    def func():
        ...
        # Fillna. For certain functions, must add other options,
...
def make_proportion_var(agent, geog, geog_id, target_variable, target_value):
    ...
    """
    var_name = 'prop_{}_{}'.format(target_variable, int(target_value))
...
def make_dummy_variable(geog_var, geog_id):
    ...
    """
    var_name = '{}_is_{}'.format(geog_var, int(geog_id))
...
def make_ratio_var(agent1, agent2, geog):
    ...
    """
    var_name = 'ratio_{}_to_{}'.format(agent1, agent2)
...
def make_density_var(agent, geog):
    ...
    """
    var_name = 'density_{}'.format(agent)
...
variables_to_aggregate = {
    'households': ['persons', 'cars', 'income', 'race_of_head', 'age_of_head',
...
discrete_variables = {
    'households': ['persons', 'cars', 'race_of_head', 'workers', 'children',
                   'tenure', 'recent_mover'],
            ...
            for var in variables:
                ...
                for aggregation_function in aggregation_functions:
                    ...
                    if aggregation_function == 'sum':
                        if var in sum_vars:
                            make_agg_var(agent, geography_name, geography_id,
                                         var, aggregation_function)
                            generated_variables.add(
                                aggregation_function + '_' + var)

                    else:
                        make_agg_var(agent, geography_name, geography_id, var,
    ...
    for var in discrete_vars:
        ...
        for cat in cats_to_measure:
            for geography in geographic_levels:
                geography_name = geography[0]
                geography_id = geography[1]
                make_proportion_var(agent, geography_name, geography_id, var,
                                    cat)
