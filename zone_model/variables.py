...
def nru_spaces(zones):
    return np.round(zones.non_residential_units*1.2) # The NRDPLCM capacity variable

@orca.column('zones', 'vacant_nru_spaces', cache=False)
def vacant_nru_spaces(zones, non_residential_units):
    return zones.nru_spaces.sub(
        non_residential_units[geography_base_id].value_counts(), fill_value=0)
